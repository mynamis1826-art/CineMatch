...
function add(m){if(!liked.some(x=>x.id==m.id)){m.rating=0;liked.push(m);}render();$('results').innerHTML='';}
...
function addUiPolish(){
    ...
    if(recommendButton&&!$('clear-all')){const button=document.createElement('button');button.id='clear-all';button.textContent='Удалить всё';button.className='mr-3 rounded-xl border border-fuchsia-500/50 bg-fuchsia-500/10 px-4 py-2 text-sm font-semibold text-fuchsia-200 hover:bg-fuchsia-500/20';button.onclick=clearAll;recommendButton.parentElement.insertBefore(button,recommendButton);}
    document.querySelectorAll("#liked button[onclick^='removeMovie']").forEach(button=>{button.className='flex h-8 w-8 items-center justify-center rounded-full bg-fuchsia-500/20 text-xl font-bold text-fuchsia-200 ring-1 ring-fuchsia-400/30 transition hover:bg-fuchsia-500/30';});
}
const originalRender=render;
render=function(){originalRender();addUiPolish();};
const originalSearch=search;
search=async function(){await originalSearch();document.querySelectorAll('#results button[onclick^="add("]').forEach(button=>{const movieId=button.getAttribute('onclick').match(/"id":(\d+)/)?.[1];if(movieId&&!button.nextElementSibling?.matches('a'))button.insertAdjacentHTML('afterend',`<a href='https://www.themoviedb.org/movie/${movieId}' target='_blank' rel='noopener noreferrer' class='ml-2 inline-flex rounded-lg border border-fuchsia-500/50 bg-fuchsia-500/10 px-3 py-2 text-xs font-semibold text-fuchsia-200 hover:bg-fuchsia-500/20'>Подробнее</a>`);});};
...
render();addUiPolish();loadSidebar();$('query').addEventListener('keydown',e=>{if(e.key==='Enter')search()});
