...
async def add_daily_movie(request, call_next):
    ...
    if request.url.path != "/" or "text/html" not in response.headers.get("content-type", ""):
        return response
    body = b"".join([chunk async for chunk in response.body_iterator]).decode("utf-8")
    daily = "<aside class='fixed bottom-5 right-4 z-20 hidden w-56 rounded-2xl border border-amber-700 bg-gradient-to-br from-amber-950 to-slate-900 p-4 text-white shadow-xl xl:block'><span class='text-[10px] uppercase tracking-widest text-amber-400'>Фільм дня</span><div id='daily-movie' class='mt-2 text-sm text-slate-300'>Завантаження...</div></aside><script>fetch('/api/daily').then(r=>r.json()).then(m=>{document.getElementById('daily-movie').innerHTML=`${m.poster?`<img src='${m.poster}' alt='' class='mb-2 h-40 w-full rounded-lg object-cover'>`:''}<strong class='block text-white'>${m.title}</strong><span class='text-xs text-amber-300'>${m.year}</span>`}).catch(()=>{document.getElementById('daily-movie').textContent='Не вдалося завантажити';});</script>"
